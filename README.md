...
Чат-додаток на MERN-стеку з підтримкою аутентифікації через Google (Clerk), асинхронної обробки подій (Inngest), інтеграцією GetStream для чатів і відеодзвінків, розгортанням на Vercel та моніторингом помилок через Sentry.
...
- **MongoDB** для збереження профілів користувачів.
- Деплой бекенду на **Vercel**.
